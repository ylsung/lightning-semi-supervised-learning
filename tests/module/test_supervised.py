...
def test_supervised(tmpdir):
    original_argv = copy.deepcopy(sys.argv)
    sys.argv = ["tmp.py", "-c", f"{CONFIG_PATH}", "--log_root", str(tmpdir)]
    print(sys.argv)
    args = parser()
    ...
    data_loader = SupervisedCIFAR10Module(
        args,
        args.data_root,
        args.num_workers,
        args.batch_size,
    ...
    model = ClassifierModule(args, classifier, loaders=None)
