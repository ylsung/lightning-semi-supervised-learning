...
class ClassifierModule(pl.LightningModule):
    ...
    def __init__(self, hparams, classifier, loaders):
        ...
        self.train_dict = {key: collections.deque([], size) \
            for key, size in zip(["loss", "acc", "val_acc", "test_acc"], \
                                 [500, 500, 1, 20])}
    ...
    def configure_optimizers(self):
        ...
        parameters = \
            split_weight_decay_weights(self.classifier, self.hparams.weight_decay, ["bn", "bias"])

        opt = torch.optim.AdamW(parameters, lr=self.hparams.learning_rate, weight_decay=0)
